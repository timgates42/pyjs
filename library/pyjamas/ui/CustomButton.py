...
-------------+---------------------+---------------------------+---------
up           |getUpFace()          |button is up               |none
down         |getDownFace()        |button is down             | up
up-hovering  |getUpHoveringFace()  |button is up and hovering  | up
up-disabled  |getUpDisabledFace()  |button is up and disabled  | up
...
class Face:
    ...
    def getFace(self):
        ...
        if self.face is not None:
            return self.face
        if self.delegateTo is not None:
...
class CustomButton (ButtonBase):
    ...
    DOWN_HOVERING = DOWN_ATTRIBUTE | HOVERING_ATTRIBUTE # 3 ID for downHovering.
    ...
    def cleanupCaptureState(self):
        ...
        """
        if not self.isCapturing and not self.isFocusing:
            return
        DOM.releaseCapture(self.getElement())
        self.isCapturing = False
        self.isFocusing = False
    ...
    def getFaceFromID(self, face_id):
        if (face_id == self.DOWN):
            ...
            raise Exception("%s is not a known face id." % str(face_id))
