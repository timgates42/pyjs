...
* @author John Kozura
"""
class RangeImplIE6 extends RangeImpl:
    ...
    @Override
    def compareBoundaryPoint(self, rng, compare, how):
    ...
    * @see com.bfr.client.selection.impl.RangeImpl#copyContents(com.bfr.client.selection.impl.RangeImpl.JSRange, com.google.gwt.dom.client.Element)
    """
    @Override
    def copyContents(self, rng, copyInto):
        copyInto.setInnerHTML(getHtmlText(rng))


    @Override
    def createFromDocument(self, doc):
        JS("""
    ...
    @Override
    def createRange(self, doc, startPoint, startOffset, endPoint, endOffset):
        """
    ...
    * @see com.bfr.client.selection.impl.RangeImpl#deleteContents(com.bfr.client.selection.impl.RangeImpl.JSRange)
    ...
    @Override
    def extractContents(self, rng, copyInto):
        copyContents(rng, copyInto)
        deleteContents(rng)


    @Override
    def fillRangePoints(self, rng):
    ...
    * @param end End range point to check
    """
    def canonicalize(self, start, end):
    ...
    @Override
    def getCommonAncestor(self, rng):
        JS("""
        return rng.parentElement();
        """)

    @Override
    def getHtmlText(self, rng):
        JS("""
        return rng.htmlText;
        """)
    ...
    * @see com.bfr.client.selection.impl.RangeImpl#surroundContents(com.bfr.client.selection.impl.RangeImpl.JSRange, com.google.gwt.dom.client.Element)
    """
    @Override
    def surroundContents(self, rng, copyInto):
        copyContents(rng, copyInto)
        Text txt = placeholdRange(rng)
        if txt is not None:
    ...
    def collapseRange(self, rng, start):
        JS("""
        rng.collapse(start);
        """)
    ...
    * @return
    """
    def createRangeOnFirst(self, parent):
        JS("""
    ...
    * @return
    """
    def createRangeOnText(self, setText, offset):
    ...
    * @return RangeEndPoint representing this, or None on error
    """
    def getRangeEndPoint(self, rng, selRange, start):
        res = None
        ...
        return (res is None) ? RangeEndPoint() : res


    def getTestElement(self, document):
    ...
    * Move both the start and end point of this range
    """
    def moveCharacter(self, rng, chars):
        JS("""
    ...
    * Move just the end point of this range
    """
    def moveEndCharacter(self, rng, chars):
        JS("""
        return rng.moveEnd("character", chars);
        """)


    def moveRangePoint(self, rng, moveTo, how):
        JS("""
        rng.setEndPoint(how, moveTo);
        """)


    def moveToElementText(self, rng, element):
        JS("""
        rng.moveToElementText(element);
        """)


    def placeholdPaste(self, rng, str):
        JS("""
        rng.pasteHTML(str);
        """)
    ...
    *
    * An alterbut far more complicated method would be to try to do
