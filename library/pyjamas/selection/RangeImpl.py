...
class RangeImpl:
    ...
    class JSRange extends JavaScriptObject:
        JSRange() {}
    ...
    * @return The value
    """
    int getIntProp(JavaScriptObject object, String propertyName)  {
        JS("""
    ...
    * @return the object
    """
    def getProperty(self, object, propertyName):
        JS("""
        var value = object[ propertyName ];
        return value || null;
        """)
    ...
    * @return -1, 0, or 1 depending on order of the 2 ranges
    """
    def compareBoundaryPoint(self, range, compare, how):
        JS("""
    ...
    * @param copyInto an element to copy these contents into
    """
    def copyContents(self, range, copyInto):
        JS("""
    ...
    * @param copyInto an element to extract these contents into
    """
    def extractContents(self, range, copyInto):
        JS("""
    ...
    * @param fillRange range object to set the endpoints of
    """
    def fillRangePoints(self, fillRange):
    ...
    * @return the lowest element that completely encompasses the range
    """
    def getCommonAncestor(self, range):
        JS("""
    ...
    * @param copyInto element to surround the range's contents with
    """
    def surroundContents(self, range, copyInto):
        JS("""
        copyInto.appendChild(range.extractContents());
    ...
    * @return A range end point with a proper (or None) text node
    """
    def findTextPoint(self, node, offset):
        RangeEndPoint res
        if node.getNodeType() == Node.TEXT_NODE:
            res = RangeEndPoint((Text)node, offset)
         else:
